# Summary

[Introduction](ch00-introduction.md)

---

# Part I: Inventory & Order Pipeline

- [1. The Distributed Inventory Ledger 🟢](ch01-distributed-inventory-ledger.md)
- [2. The Order Orchestrator (DAG Workflow) 🟡](ch02-order-orchestrator-dag.md)

---

# Part II: Warehouse & Edge

- [3. Warehouse Robotics and Edge Computing 🟡](ch03-warehouse-robotics-edge.md)

---

# Part III: Routing & Intelligence

- [4. The Last-Mile Routing Algorithm (VRP) 🔴](ch04-last-mile-routing-vrp.md)
- [5. Demand Forecasting and Inventory Placement 🔴](ch05-demand-forecasting-placement.md)
